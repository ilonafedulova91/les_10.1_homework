...
def test_filter_by_currency_success(transactions):
...
def test_filter_by_currency_errors():
    with pytest.raises(ValueError):
        list(filter_by_currency([], 'USD'))
    with pytest.raises(ValueError):
        list(filter_by_currency([{'operationAmount': {'amount': '56883.54', 'currency': {'name': 'USD', 'code': ''}}}], 'USD'))
    with pytest.raises(TypeError):
        list(filter_by_currency([{'operationAmount': {'amount': '56883.54', 'currency': {'name': 'USD', 'code': '10'}}}], 'USD'))
        list(filter_by_currency([{'operationAmount': {'amount': '56883.54', 'currency': {'name': 'USD', 'code': 10}}}], 'USD'))
...
def test_transaction_descriptions_errors():
    with pytest.raises(ValueError):
        list(transaction_descriptions([]))
    with pytest.raises(ValueError):
        list(transaction_descriptions([{"description": ""}]))
    with pytest.raises(ValueError):
        list(transaction_descriptions([{}]))
...
def test_card_number_generator_errors():
    with pytest.raises(ValueError):
        list(card_number_generator(-2, 5))
    with pytest.raises(ValueError):
        list(card_number_generator(0, 5))
    with pytest.raises(ValueError):
        list(card_number_generator(5, 9999999999999999999))
